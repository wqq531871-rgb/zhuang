...
放箱顺序。普通托盘按几何层逐层执行，每层按原点方向的 X 列、列内 Y 递增扫描；
只有同一几何层内相邻箱顶的显著高度跳变同时达到数量和比例门槛时，才改用空间
阶梯波，并在同一波次优先铺设较低支撑层。安全依赖或防包围门禁需要偏离扫描时，
规划器会记录对应箱号和原因。执行规划还会在托盘有剩余空间的方向居中整个垛型，
