...
> 接口编号统一对标接口文档第 4 章：**4.1～4.7**（见 §1 总表与 §6 TODO）。
> 注意：本文自身章节号（如「§3 输入适配」「§4 输出适配」）与接口文档的 4.x **不是同一套编号**。
> 本文档以分析与接入方案为主；字段口径对照当前算法与适配层核实。待确认项在 §8 与 TODO(4.5/4.6/4.7) 列出。
> 2026-07-11 增补：§9 增量/分批订单对接、§10 指数数据本地配置、§11 算法配置
> 的系统对接（对照已实装的 `src/incremental/` 与 `src/adapter/wcs_adapter.py`）。
> 2026-07-24 增补：接口编号 4.1～4.7、TODO(4.5 托盘更新 / 4.6 托盘到达 / 4.7 系统状态判定)。

---

## 1. 接口文档概览（对标接口文档第 4 章：4.1～4.7）

WCS 与机器人系统（我方，算法宿主）通过 **HTTP + JSON** 相互通讯（4.7 为 GET，其余多为 POST）。

| 接口文档编号 | 原称 | 方向 | 地址 | 与装箱算法的关系 | 落地状态 |
|---|---|---|---|---|---|
...
| **4.5** | 托盘更新 | 机器人→WCS | `/adaptor/api/wcs/reqpallet` | 执行层（机械臂向 WCS 要/报托盘） | **TODO(4.5)** 暂不实现出站；见下文 |
| **4.6** | 托盘到达 | WCS→机器人 | `/adaptor/api/wcs/palletarrive` | 执行层（托盘送达站台） | 接收端已接；**暂仅按示例回成功**；**TODO(4.6)** 业务语义待对齐 |
| **4.7** | 获取系统信息 | WCS→机器人（**GET**，最快约 1s/次） | `/api/status` | 设备状态轮询 | 接收端已维护；**TODO(4.7)** 空闲/有任务判定待定 |

**只有 4.1、4.2 直接进出装箱算法**；4.3～4.7 属机械臂执行/物流控制层，不触碰算法核心，但依赖算法结果（通过 `box_unique_id` 关联，见后文执行层说明）。
...
│  ├─ POST 4.1 reqstockinfo ──输入─┐ ├─ POST 4.3 sendcasetask          │
│  ├─ POST 4.2 sendpalletplanresult┼─├─ POST 4.4 boxarrive             │
│  └─ POST 4.5 reqpallet（TODO）   │ ├─ POST 4.6 palletarrive          │
│         ▲                        │ └─ GET  4.7 /api/status           │
│         │ 输出(层/顺序视图)       │                                   │
│  ┌──────┴────────────────────────▼──────────────────────────┐        │
│  │ 接口适配层（算法核心零改动）                                 │        │
...
- **传输方式**：HTTP（4.7 为 GET），请求/响应体均为 JSON；统一响应 `{code:0成功/非0失败, msg, data}`。
- **算法调用方式**：进程内 `run_with_boxes(boxes)`（现有公开入口），不落地 Excel、不经 CLI。
- **触发时机**：规划阶段由机器人侧**主动**发起（4.1 是机器人→WCS）。即"拉库存 → 规划 → 推结果"由我方定时/批次/人工触发；WCS 不推订单（创建订单接口已删除）。

### 2.2 数据流时序（规划阶段，与算法直接相关）

1. 机器人侧 POST **4.1** `reqstockinfo`（带 `msgtime` + 32 位 `msgid`）→ WCS 返回**按品类聚合**的库存数组；
2. 适配层把库存转成算法箱子列表（§3），调用 `run_with_boxes(boxes)` 得到完整装箱报告（含每箱坐标）；
3. 适配层把报告转成 **4.2** 的 case 数组（§4），POST `sendpalletplanresult` 给 WCS；完整报告（坐标版）在机器人侧留存，按 `box_unique_id` 建索引；
4. 执行阶段（不经算法）：WCS 按规划顺序出库 → **4.3** 预告 case → **4.4** 逐箱到达（带 `seq`）→ 机械臂按留存坐标码垛 → **4.5/4.6** 托盘轮换 → **4.7** 状态轮询。

---

## 3. 输入适配：接口文档 4.1（库存信息获取）→ 算法箱子列表

### 3.1 4.1 返回的库存条目（每元素 = 一个**品类**，非逐箱）
...
## 4. 输出适配：算法报告 → 接口文档 4.2（规划订单输出）

> 本节标题「§4」是本文章节号；接口本身编号为 **4.2**。下文「§4.1 / §4.2 / …」指本文小节，勿与接口文档 4.1/4.2 混淆。

### 4.1 4.2 要求的输出（数组，每元素 = 一个拼好的 case/托盘）
...
## 6. 执行层接口（4.3～4.7）与算法的关系

这些接口**不进算法**，但适配层/机器人服务需要：

| 接口文档编号 | 需要算法侧 / 执行侧提供什么 |
|---|---|
| **4.3** 拼箱物料信息下发 | **记录选定托盘**（`write_selected_pallet_session`）→ 现场码垛显示「托盘已选定」、三维可整盘加载；目标后续再按 `box_unique_id` 取完整坐标方案 |
| **4.4** 物料到达 | 目标：按 `box_unique_id + seq` 定位并登记。**当前联调**：收到请求即回 `{code:0,msg:success,data:{}}`，暂不登记/不驱动 PLC |
| **4.5** 托盘更新 | 机械臂向 WCS 请求新托盘（我方出站）。**TODO(4.5)** 暂不实现；见 §6.1 |
| **4.6** 托盘到达 | WCS 告知机械臂托盘已送达当前站台。**当前联调**：收到请求即回成功示例；业务状态机 **TODO(4.6)** 见 §6.2 |
| **4.7** `/api/status` | 设备状态（0 就绪 / 1 执行中 / 99 异常），纯执行层状态机。**TODO(4.7)** 见 §6.3 |

**结论**：算法输出的 JSON 报告已包含执行层所需的全部信息（坐标、真实尺寸、
吸盘姿态、码垛顺序），无需为执行层扩充算法输出——适配层留存映射即可。

### 6.1 TODO(4.5) 托盘更新 — 机器人 → WCS

- **业务场景**：机械臂向 WCS 请求新托盘（并可附带当前托盘上的码放信息）。
- **地址**：`POST /adaptor/api/wcs/reqpallet`
- **发送时机**：**TODO(4.5-时机)** 具体何时发送待定（例如：当前托盘码满 / 需要空托盘 / 站台切换等）。
- **我方发送示例**（`case_data` 按层；每层 `cartons` 含 length/width/height/product_code；字段可按实际垛层扩展）：
...
```json
{
  ...
  "case_data": [
    {
      "cartons": [
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 10791358 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 28213276 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 50987442 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 10791358 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 28213276 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 50987442 },
        { "length": 700.0, "width": 530.0, "height": 360.0, "product_code": 10791358 },
...
- **落地 TODO**：出站 HTTP 客户端封装、与执行进度（已码 cartons / empty_flag）拼装、失败重试与日志。

### 6.2 TODO(4.6) 托盘到达 — WCS → 机器人

- **业务场景**：WCS 告知机械臂托盘已经送达当前站台。
- **地址**：`POST /adaptor/api/wcs/palletarrive`
- **对方发送示例**：
...
- **落地状态**：本地接收端已接；**当前仅回上述成功示例，不做业务处理**（请求仍可落盘日志）。
- **TODO(4.6-业务)**：与站台/机械臂状态机对齐（收到后是否切换当前托盘、是否清上次垛型上下文、是否与 4.5 成对）；请求字段是否仅以上述四项为准（若对方仍带 `case_data`/`empty_flag` 需兼容）。

### 6.3 TODO(4.7) 获取系统信息 — WCS → 机器人（GET）

- **业务场景**：WCS 获取机械臂系统状态，以判断是否要推送 case 到机械臂码垛。
- **地址**：`GET /api/status`（注意无 `adaptor` 前缀）
- **调用频率**：最快约 **1s 请求一次**
- **我方回复示例**：
...
- **落地状态**：接收端已维护（当前可由 `receiver_config.yaml` 的 `device_status` 配置返回）。
- **TODO(4.7-判定)**：具体何时算「空闲 / 有任务」待定；后续需改为真实状态机写入，而非仅静态配置。
...
2. **HTTP 服务壳（待现场框架）**：出方向 **4.1 / 4.2 / TODO(4.5)**；入方向 **4.3 / 4.4 / 4.6** + **GET 4.7**；统一 `{code, msg, data}` 响应；4.3 的 box_unique_id 去重由服务壳负责。
