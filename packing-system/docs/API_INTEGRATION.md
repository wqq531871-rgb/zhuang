...
WCS 接口库存模式会完整保存接口 1 的原始响应用于审计；在写入
`wcs_stock_box` 和 `wcs_stock_box_all` 之前，会忽略 `length`、`width`、
`height` 任一项缺失、非数字、非有限值或小于等于 0 的记录。被忽略记录不参与
装箱，日志会记录数量、`product_code`、箱型和原始尺寸。输出质量门禁仍会继续
拦截其它路径产生的零尺寸结果。如果本次目标箱型记录全部因非法尺寸被过滤，
当前库存表会清空，避免沿用上一次的旧库存；WCS 真正返回空包时仍保留原有的
防空包保护。
