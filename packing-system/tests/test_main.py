...
from src.execution import ExecutionSequenceConfig
...
def test_report_persister_writes_pallet_excel_summary():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert not list(Path(tmpdir).glob("*_execution*.json"))


def _execution_test_box(box_id, x, height, product_code):
    return {
        "id": box_id,
        "type": "T",
...
def test_report_persister_writes_execution_bundle_when_enabled(tmp_path):
    persister = JsonFileReportPersister(
        tmp_path,
        lambda fmt: "20260720_120000",
        execution_sequence_config=ExecutionSequenceConfig(),
    )
    tall = _execution_test_box("tall", 0, 300, 1)
    short = _execution_test_box("short", 200, 100, 2)
    report = {
        "pallets": [{
            "pallet_id": "P1",
            "pallet_type": "TEST",
            ...
            "case_group": 0,
            "mpm_status": "SUCCESS",
            "packed_items": [tall, short],
        }]
    }

    persister.persist(report, 1.0)

    original_path = tmp_path / "packing_plan_20260720_120000.json"
    execution_path = tmp_path / "packing_plan_20260720_120000_execution.json"
    wcs_path = tmp_path / "packing_plan_20260720_120000_execution_wcs.json"
    map_path = tmp_path / "packing_plan_20260720_120000_execution_wcs_map.json"
...
def test_run_packing_loads_execution_planner_config(tmp_path, enabled, expected):
    ...
    config = run_packing.load_execution_planner_config(config_path)
    ...
    if expected is None:
        ...
        assert config.approach_z_clearance_mm == 43.0
        assert config.approach_box_xy_clearance_mm == 44.0
        assert config.approach_suction_xy_clearance_mm == 45.0
