...
class JsonFileReportPersister:
    ...
    def __init__(
        ...
        execution_sequence_config: Optional[ExecutionSequenceConfig] = None,
    ):
        self._output_dir = output_dir
        self._timestamp_fn = timestamp_fn
        self._execution_sequence_config = execution_sequence_config
    ...
    def persist(self, report: Dict, total_runtime: float) -> None:
        ...
        if execution_paths is not None:
            print(f"机械臂执行方案已保存至：{execution_paths.execution}")
            print(f"WCS接口文件已保存至：{execution_paths.wcs_cases}")
            print(f"机器人执行映射已保存至：{execution_paths.wcs_map}")
        print("=" * 40)
