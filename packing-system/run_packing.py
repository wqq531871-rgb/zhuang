...
def load_execution_planner_config(config_path=None):
    """加载启用后的独立执行顺序规划参数；关闭时返回 None。"""
    if config_path is None:
        default_yaml = project_root / 'config' / 'packing_config.yaml'
        config_path = default_yaml if default_yaml.exists() else None
    if config_path is None or not Path(config_path).exists():
...
def build_workflow(
    ...
    execution_sequence_config: ExecutionSequenceConfig = None,
) -> PackingWorkflow:
    ...
    return PackingWorkflow(
        ...
        report_persister=JsonFileReportPersister(
            ...
            execution_sequence_config=execution_sequence_config,
        ),
...
def _run_incremental(
    ...
    incr_filepath,
    out_path,
    execution_sequence_config=None,
):
    ...
    JsonFileReportPersister(
        OUTPUT_DIR,
        lambda fmt: pd.Timestamp.now().strftime(fmt),
        execution_sequence_config=execution_sequence_config,
    ).persist(report, result.total_runtime_seconds)
    if out_path:
...
def _run(out_path, max_boxes, safe_compare=False, config_path=None):
    ...
    execution_sequence_config = load_execution_planner_config(config_path)
