...
|---|---|
| **4.3** 拼箱物料信息下发 | 目标：按 `box_unique_id` 取完整方案。**当前联调**：收到请求即回 `{code:0,msg:success,data:{}}`，暂不查方案/不落库 |
| **4.4** 物料到达 | 目标：按 `box_unique_id + seq` 定位并登记。**当前联调**：收到请求即回成功示例，暂不登记/不驱动 PLC |
| **4.5** 托盘更新 | 机械臂向 WCS 请求新托盘 / 上报当前托盘上已码 cartons（可从执行进度生成），与规划算法无关。**TODO(4.5)** 见 §6.1 |
