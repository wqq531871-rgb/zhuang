...
def test_cpsat_selects_minimum_overshoot_target_subset():
    from src.packing.global_column_packer import _select_target_subset_cpsat

    boxes = [
        *_mk('A', 1, 100, 100, 100, 180),
        *_mk('B', 1, 100, 100, 100, 12),
        *_mk('C', 1, 100, 100, 100, 20),
    ]
    ...
    assert {box['id'] for box in selected} == {'A0', 'B0'}
    assert sum(box['min_pack_multiple'] for box in selected) == 192


def test_exact_target_subset_placement_uses_every_selected_box():
    pallet = {'length': 200.0, 'width': 100.0, 'height': 200.0}
    boxes = [
        *_mk('E', 2, 100, 100, 100, 100),
    ]
    for box in boxes:
        box['pallet_dims'] = pallet
    ...
    board = gcp._place_exact_target_subset(
        'TEST', 'O1', boxes, 192.0, pallet,
    )

    assert board is not None
    assert {box['id'] for box in board['packed_items']} == {'E0', 'E1'}
    assert board['mpm_status'] == 'SUCCESS'
    gate = validate_pallet_constraints(
        board,
        pallet,
...
def test_exact_target_candidate_retries_after_geometry_failure(monkeypatch):
    import src.packing.global_column_packer as gcp_module

    boxes = _mk('N', 4, 100, 100, 100, 100)
    gcp = GlobalColumnPacker(constraint_config=ConstraintConfig(
        suction_reachability_enabled=False,
    ...
    monkeypatch.setattr(
        gcp_module, '_select_target_subset_cpsat', fake_select
    )
    placement_calls = []

    def fake_place(*args, **kwargs):
        selected = args[2]
        placement_calls.append([box['id'] for box in selected])
    ...
    monkeypatch.setattr(gcp, '_place_exact_target_subset', fake_place)
    monkeypatch.setattr(
        gcp_module,
        '_gcp_candidate_passes_gates',
        lambda *_args, **_kwargs: True,
    ...
    monkeypatch.setattr(
        gcp,
        '_pack_group_with_columns',
        lambda *args, **kwargs: (
            [{
                'packed_items': boxes[:2],
                'mpm_total': 200.0,
                'mpm_status': 'SUCCESS',
            }],
            {'packing': 0.01},
    ...
    result = gcp._build_exact_target_candidate(
        'MH423C', 'T', boxes, 192.0, PALLET,
    )

    assert result is not None
    _plans, _runtime, diag = result
    assert placement_calls == [['N0', 'N1'], ['N2', 'N3']]
    assert diag['cpsat_target_subset_attempts'] == 2
    assert diag['cpsat_target_subset_geometry_failures'] == 1
    assert frozenset({'N0', 'N1'}) in exclusions_seen[1]


def test_pack_group_adds_exact_subset_candidate_for_uncaptured_upper_bound(
    monkeypatch,
):
    import src.packing.global_column_packer as gcp_module

    boxes = _mk('U', 2, 100, 100, 100, 100)
    gcp = GlobalColumnPacker(constraint_config=ConstraintConfig())
    monkeypatch.setattr(
        gcp_module,
        '_build_column_candidates',
        lambda *_args, **_kwargs: [('height_first', [{'token': 'height'}])],
    )
    monkeypatch.setattr(
        gcp_module,
        '_gcp_candidate_passes_gates',
    ...
    def fake_exact(*args, **kwargs):
        ...
        return (
            exact_plan,
            {'packing': 0.02},
            {'gcp_bailout': False, 'gcp_column_strategy': 'cpsat_target_subset'},
        )

    monkeypatch.setattr(gcp, '_build_exact_target_candidate', fake_exact)

    plan, _runtime, diag = gcp.pack_group('MH423C', 'T', boxes, 192.0)

    assert len(exact_calls) == 1
    assert plan is exact_plan
    assert diag['gcp_selected_column_strategy'] == 'cpsat_target_subset'
