...
`/adaptor/api/wcs/sendcasetask` 选定托盘后自动启动该托盘的状态监听和 PLC
发送；界面同时保留“手动启动当前托盘”按钮，供联调和 WCS 不可用时使用。

通讯能力随 `packing-robot` 主程序加载，但不得在程序启动时自动连接或写入
PLC。操作员加载当前托盘、确认 PLC 参数后，手动连接并启动整盘发送。
...
4. 若界面启用“WCS 自动启动 PLC”，加载成功后启动当前托盘的后台 PLC 任务。
...
程序启动、加载托盘或数据库 `state` 变为非空，都不会单独触发 PLC 写入；只有
操作员明确点击“开始发送”后，后台发送线程才有写 PLC 的权限。
...
- 增加“WCS 自动启动 PLC”开关，默认开启并持久化设置。
- 增加“手动启动当前托盘”按钮和“停止”按钮。
- 收到接口3产生的 `load_pallet` 指令并成功加载托盘后，自动模式启动后台任务。
